...
function HaberYapici(haber) {
  ...
  pTarih.textContent = haber.tarih;
  article.appendChild(pTarih);
...
/*
Adım 5: Yeni bir teknoloji haberi ekle (aynı yapı)
*/
data.push({
  baslik:
    'Open-source LLM yarışında yeni adım: Topluluk destekli modeller yükselişte',
  tarih: '2 Ekim 2025',
  ilkParagraf:
    'Açık kaynak büyük dil modelleri; hız, maliyet ve esneklik avantajlarıyla şirketlerin PoC aşamasından üretime geçişini hızlandırıyor.',
  ikinciParagraf:
    'Uzmanlar, modüler mimari ve kuantizasyon teknikleri sayesinde edge cihazlarda dahi inference yapılabildiğini, veri egemenliği ihtiyacı olan sektörlerin OSS modellere yöneldiğini belirtiyor.',
  ucuncuParagraf:
    'Gözler, lisanslama netliği ve güvenlik değerlendirmelerinde. Kurumlar, model kartları ve değerlendirilebilir benchmark raporlarını karar süreçlerinde şart koşuyor.',
});

/*
Adım 4: data’yı döngüye sok, .articles içine ekle
*/
const articlesRoot = document.querySelector('.articles');
data.forEach((haber) => {
  const node = HaberYapici(haber);
  articlesRoot.appendChild(node);
});
